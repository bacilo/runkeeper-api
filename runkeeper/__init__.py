...
class RunKeeper():
	def __init__(self, client_id, client_secret, access_token="", redirect_uri='http://localhost:8000/gettoken.html'):
	...
	def get_all_activities_csv(self, output_folder=''):
		...
		w_acts = csv.writer(f_acts, dialect='excel', encoding='utf-8', delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
		w_act_data = csv.writer(f_act_data, dialect='excel', encoding='utf-8', delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
	...
	def get(self, endpoint, media_type):
		return self.oauth.get(API_URL + endpoint,
			headers={
			'Authorization': 'Bearer ' + self.access_token,
			'Accept': 'application/vnd.com.runkeeper.{}+json'.format(media_type) 
			})

	# An attempt to automate the getting of the token. Still an ambition to do but not a priority:
	...
	# 	self.access_token = self.oauth.fetch_token(self.access_token, authorization_response=authorization_response, client_secret=self.client_secret)
